...
function Home() {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      const { data } = await axios.post("http://doodl-backend.vercel.app:5000/api/create-room", {
        playerName,
      });
  ...
  const handleJoinRoom = async () => {
    ...
    if (roomId) {
      try {
        const response = await axios.post("http://doodl-backend.vercel.app:5000/api/room-exists", { roomId });
